...
def build_user_prompt(
    ...
    header = f"종목: {stock_name} ({symbol})\n\n"
    if is_sector_fallback:
        # 섹터 fallback: 종목 직접 뉴스가 없어 섹터 관련 뉴스로 대체. summary 에서 이 한계를
        # 명시하도록 강제하고, key_reasons 는 "섹터 관련" 표현을 반드시 포함하게 유도한다.
        header += (
            f"⚠️ 이 종목 ({stock_name}) 직접 뉴스가 부족해 같은 섹터의 최근 뉴스를 참고합니다.\n"
            "summary 첫 문장에 '해당 종목 직접 관련 뉴스가 부족합니다' 명시. "
            "key_reasons 는 반드시 '섹터/업종 동향' 관점으로만 작성하고, "
            "종목의 개별 이슈를 단정하지 마세요.\n\n"
        )
    return (
        f"{header}"
