...
인용 규칙 (매우 중요):
- key_reasons 의 **모든 항목**은 어느 뉴스에서 뒷받침되는지 `[뉴스 N번]` 마커를 반드시 포함해야 합니다.
- 여러 뉴스가 근거이면 `[뉴스 1번, 3번]` 처럼 나열합니다.
- 뉴스에서 직접 뒷받침되지 않는 근거는 **만들지 마세요**. 근거가 부족하면 key_reasons 를 짧게 반환하는 것이 정답입니다.
- summary 문장 안에서도 특정 사실 (숫자, 계약, 실적) 을 언급할 땐 어떤 뉴스에서 나왔는지 `[뉴스 N번]` 을 붙입니다.
